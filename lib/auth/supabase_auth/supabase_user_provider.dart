...
class SpendifyAppV2SupabaseUser extends BaseAuthUser {
  SpendifyAppV2SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> spendifyAppV2SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SpendifyAppV2SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
